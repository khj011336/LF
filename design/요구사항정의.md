...
- String adress 	주소
- String ticket 	<<uq>>	보유 티켓	(16자 무작위)
- int payment 		사용중인 이용권	(1:전체권 2:3개권 3:1개권 ...)
- Date joinedAt 	가입일
...
- varchar(12) salt		salt값		NN
- varchar(12) ph_number 휴대전화
- varchar(128) adress 	주소
- varchar(17) ticket 	보유 티켓		<<UK>>	(16자 무작위)
- tinyint payment	 	사용중인 이용권	
- timestamp joined_at	가입일 		NN
- varchar(128) shopCart	장바구니		("kit_id,kit_id,kit_id" 방식으로 저장
- tinyint agreeReceive 	수신동의 		(0:선택x 1:1번항목체크 2:2번항목체크 3:모두체크) 
...
- varchar(128) sumnail	썸네일 
- varchar(128) subTitle	한줄소개
- varchar(256) introduce	강의소개글
- varchar(128) introducePic	강의소개글사진
- varchar(256) introduceCreater	작가소개글	
- timestamp created_at	등록날자
- varchar(12) like_mb	좋아요한회원	("mb_id,mb_id,mb_id,..."방식으로 저장)
- varchar(12) will_mb	찜하기한회원	("mb_id,mb_id,mb_id,..."방식으로 저장)
- varchar(12) show_mb	이용중인회원	("mb_id,mb_id,mb_id,..."방식으로 저장)
...
- tinyint table		댓글달리는 게시판 위치(1:classes 2:videos 3:postscripts 4:kits 5:notices 6:qnas)
- int parents_id	부모번호	<<fk>>	(댓글을 다는 주체글)
- int order			순서번호
- int depth			댓글깊이
